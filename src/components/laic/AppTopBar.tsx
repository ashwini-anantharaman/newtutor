  ...
  return (
    <header className={`h-[69px] shrink-0 flex items-center gap-4 px-6 ${student.topBar}`}>
      ...
      <div className={`flex items-center gap-2 px-4 py-2 rounded-full w-56 ${student.searchBg}`}>
        <Search className="w-4 h-4 text-[#a3b3ff] shrink-0" />
        <input
          type="search"
          placeholder="Search your course..."
          className="flex-1 bg-transparent text-sm font-semibold text-[#314158] placeholder:text-[#a3b3ff] focus:outline-none min-w-0"
        />
      </div>
      <button
        type="button"
        className={`relative w-9 h-9 rounded-full ${student.searchBg} flex items-center justify-center text-[#62748e] hover:bg-[#e0e7ff] transition-colors`}
